...
import Random from './components/RandorImg'
...
class App extends Component {
  ...
  updateTime = () => {
    ...
    if (time === 1) {
      ...
      this.setState(preState => ({isGameStart: !preState.isGameStart}))
    }
  }

  updateTabId = tabId => this.setState({activeItemId: tabId})

  getFilteredItem = () => {
    const {activeItemId} = this.state
    const filteredItem = imagesList.filter(
      each => each.category === activeItemId,
    )
    return filteredItem
  }

  onClickImg = id => {
    const {num, activeItemId, time, score, isGameStart} = this.state

    const TimeCompleted = time === 0

    if (TimeCompleted) {
      this.setState({isGameStart: false})
    }
    if (imagesList[num].id === id) {
    ...
    this.setState({num: Math.ceil(Math.random() * imagesList.length)})
  }

  playAgain = () => {
    const {activeItemId, num, score, time, isTimerRunning} = this.state
  ...
  gameOverCard = () => {
    ...
    return (
      <div className="cardBg">
        ...
        />
        <p className="scr-p1">YOUR SCORE</p>
        <p className="scr-p2">{score}</p>
        <button onClick={this.playAgain} type="button" className="button">
  ...
  renderGame = () => {
    ...
    const {activeItemId, num, score, time, isGameStart} = this.state
    ...
    return (
      <div className="card">
        <div className="img-card">
          <img
            alt="match"
            id={imagesList[num].id}
            className="shfimg"
            src={imagesList[num].imageUrl}
          />
        </div>
        <ul className="tab">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              isActive={tabDetails.tabId === activeItemId}
            />
          ))}
        </ul>

        <ul className="image">
          {filteredItem.map(eachImg => (
            <ImageItem
              key={eachImg.id}
              imgDetails={eachImg}
  ...
  render() {
    ...
    return (
      <div className="bg-container">
        ...
        {isGameStart ? this.renderGame() : this.gameOverCard()}
      </div>
    )
  }
}
